...
pretty_t <- function(x) {
    ...
    zero_model <- zero_model %>% mutate(pp = paste0(round(Estimate, 3),"<br>",
                                                    " (", round(`Std. Error`, 3), ")[", round(`Pr(>|z|)`, 3), "]"))
    zero_model <- zero_model[c("vars", "pp")]
    row.names(zero_model) <- NULL
    zero_model
}

to_frame_multi <- function(aa) {
    Reduce(function(x, y) merge(x, y, by = "vars", all = T, sort = F), x = aa)
}
