...
## Table 1. Number of participants for each of the 3 × 2 experimental conditions (Reproducible Results)
...
## Table 2. Differences across treatments of socio-demographic characteristics (Reproducible Results)
...
cat("Mean and SD for the distributions of 'kept' across different treatment groups within the Standard Dictator Game.\n")
...
# Explain what the Kruskal-Wallis test is applied on
cat("Mean and SD for the distributions of 'kept' across different treatment groups within the Bully Dictator Game.\n")
# Print the adjusted data
...
# Explain what the Kruskal-Wallis test is applied on
cat("Mean and SD for the distributions of 'kept' across different treatment groups within the Dictator Game.\n")
# Print the adjusted data
...
## Replicating  the drop_general variable to see how many instances are dropped generally
### when the indicator == 1; this means the individual has dropped from experiment
...
## Replicating the drop_on_task variable to see how many instances are dropped on task
### when the indicator == 1; this means the individual has dropped from experiment
...
# Replication:
> As per author's observation, we also see that such findings are not washed out by controlling for elicited variables,
> which in turn appear not to be affected by the experimental conditions.
...
## B.1 Effectiveness of cognitive manipulation (fig. 1) (Research Paper)
...
## B.1 Effectiveness of cognitive manipulation (fig. 1) (Reproducible Results)
...
## B.1 Effectiveness of cognitive manipulation (fig. 2) (Research Paper)
...
## B.1 Effectiveness of cognitive manipulation (fig. 2) (Reproducible Results)
...
summary_dg_no_interaction_df
summary_dg_interaction_df
```

Table 3 the results have differences with the paper
...
```{r hurdle}
# Declaring models
h1 <- hurdle(formula = give ~ game_type + treatment_group + crt_right + as.factor(female) + household_income, data = df)
h2 <- hurdle(formula = give ~ game_type + treatment_group + crt_right + as.factor(female) + household_income + ee + nb + ne, data = df)
h3 <- hurdle(formula = give ~ game_type + treatment_group + crt_int + as.factor(female) + household_income, data = df)
h4 <- hurdle(formula = give ~ game_type + treatment_group + crt_int + as.factor(female) + household_income + ee + nb + ne, data = df)
